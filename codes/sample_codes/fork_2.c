...
main()
{
	...
	*/
	if ((pid1 = fork()) == 0) {
        ...
        printf("first child process complete\n");
	}
	else {
		/* Parent process */
        ...
        printf("second child process complete\n");
		}
		else {
			/* Parent process */
			for (i=0 ; i < 5; i++) {
			...
			/* 
                The waitpid() system call causes the parent to wait for a child process with a specific pid to complete
                its execution. The input parameter can specify the PID of the child process for which it has to wait.
			*/
