...
void create_workers(pthread_t *tid, tinfo *param)
{
	...
	for (i = 0; i < N; ++i)
	{
		...
		param[i].tname[4] = '\0'; 	// limited to 16 characters including the null terminator (\0)
	...
	sleep(1);
	pthread_attr_destroy(&attr);  // the thread attributes object attr is no longer valid and should not be used.
	// this does not affect any threads that were created with these attributes. When you create a thread with pthread_create(), 
	// the attributes specified by the attr object are copied and used for that thread.
}
...
void create_mutex()
{
	...
	/**
	 * Thi section of code is responsible for unlocking the mutexes.
